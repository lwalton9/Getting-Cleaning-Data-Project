Getting and Cleaning Data Course Project##READ IN DATA FROM PERSONAL DIRECTORY WHERE FILE WAS STORED
getwd()
setwd("G:/LORI/2016 Role/Analytics & Data Division/Miscellaneous/Coursera R/UCI HAR Dataset/")
##READ ACTIVITY 
activityType<-read.table('./activity_labels.txt')##READ FEATURESfeatures<-read.table('./features.txt')
##READ TRAIN
xtrain<-read.table('./train/X_train.txt')ytrain<-read.table('./train/Y_train.txt')
##READ TRAIN SUBJECT
trainsubject<-read.table('./train/subject_train.txt')
###ASSIGN COLUMN NAMES TO TRAIN
colnames(xtrain)<-features[,2]colnames(ytrain)<-"activityID"colnames(trainsubject)<-"subjectID"
##READ TEST
xtest<-read.table('./test/X_test.txt')ytest<-read.table('./test/y_test.txt')
##READ TEST SUBJECT
testsubject<-read.table('./test/subject_test.txt')
###ASSIGN COLUMN NAMES TO TRAIN
colnames(xtest)<-features[,2]colnames(ytest)<-"activityID"colnames(testsubject)<-"subjectID"
###ASSIGN COLUMN NAMES To ACTIVITY
colnames(activityType)<-c("activityID","activityType")
####MERGE THE TRAINING AND THE TEST SETS TO CREATE ONE DATA SET
train<-cbind(ytrain,trainsubject,xtrain)test<-cbind(ytest,testsubject,xtest)traintest<-rbind(train,test)
#####CREATE COLUMN NAMES OF FINAL SET
colNames<-colnames(traintest)View(traintest)
...
& !grepl("-std..-",colNames))finaltraintest=traintest[ ,meanstd==TRUE]
View(finaltraintest)dim(finaltraintest
)######ADD ACTIVITY LABELS
finaltraintest_wact <- merge(finaltraintest, activityType, by='activityID',all.x=TRUE)
##REVIEW DATA
dim(finaltraintest_wact)
summary(trainsubject)summary(testsubject)
######CLEAN UP NAMES############
